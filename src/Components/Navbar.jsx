       ...
       <div className="container flex items-center justify-between">
            ...
            <div className="hidden md:flex space-x-8">
                {navItems.map((item,key)=>(
                    <a key={key} href={item.href} className="text-forefround/80 hover:text-primary transition-colors duration-3">
                        {item.name}
                    </a>
                ))}
            </div>
            ...
            <div className={cn("fixed inset-0 bg-background/95 backdrop-blur-md z-40 flex flex-col items-center justify-center",
                ...
                {navItems.map((item,key)=>(
                    <a key={key} href={item.href} className="text-forefround/80 hover:text-primary transition-colors duration-300"
